  ...
  return (
    ...
    <div className='max_padd_container flex w-full flex-col gap-14'>
      <div className='flex flex-col items-start justify-center gap-[10%] md:flex-row'>
        <Link to="/" className='mb-6 font-semibold text-[40px] '> Shopee</Link>
        <div className='flex flex-wrap gap-10 sm:justify-between md:flex-1;'>
          ...
          <div className='flex'>
          <FooterColumn>
            <ul className='flex max-md:hidden gap-4'>
              {SOCIALS.links.map((link) => (
                <Link to='/' key={link}> <img src={link} alt="socialIcon" height={22} width={22} />
                </Link>
              ))}
            </ul>
          </FooterColumn>
      ...
      <p className='text-center regular-14 text-gray-30 '> 2024 Shoppee | All rights reseved </p>
    </div>
    </footer>
  )
}
