      ...
      <div className="px-3 flexBetween py-3  max-xs:px-2">
        ...
        <div className=" flexBetween sm:gap-x-3 bold-16">
          ...
          <div className="flexBetween sm:gap-x-6 ">
            <NavLink to={"cart-page"} className={"flex"}> <FaOpencart className="p-1 h-8 w-8 ring-slate-900/30 ring-1 rounded-full"/> <span className=" relative flex-center w-5 h-5 rounded-full  bg-secondary text-white medium-14 -top-2 text-center">{getTotalCartIitems()}</span> </NavLink>
            { localStorage.getItem('auth-token') ? <NavLink to={"logout"} onClick={()=> {localStorage.removeItem('auth-token'); window.location.replace('/')}} className={"btn_secondary_rounded flexCenter gap-x-2 medium-16"}> <img src={logout} alt="logoutIcon" height={19}
              width={19}
            /> Logout </NavLink> :
